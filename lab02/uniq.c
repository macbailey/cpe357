...
int main(int argc, char *argv[])
{
	...
	/*allocating a single memory slot of size char since we only have to hold one memory*/
    ...
    {	
		...
		if(currentPosition == maxBuffer)
		{
			...
			currentLine = (char *)realloc(currentLine - currentPosition, maxBuffer); 
		...
		{	
			...
			*currentLine = '\0';
			...
			/*for the first line ensure that the last line is allocated with memory and
			copy in the current line, since it can't be a duplicate*/
			if(lastLine == NULL)
			{
				lastLine = (char *)realloc(lastLine, currentPosition + 1);
			...
			and allocate one more for the end of line terminator*/
			lastLine = (char *)realloc(lastLine, currentPosition + 1);
			currentPosition = 0; 
			strcpy(lastLine, currentLine);
